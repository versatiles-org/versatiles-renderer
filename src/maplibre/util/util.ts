...
export function asyncAll<Item, Result>(
	...
	array.forEach((item, i) => {
		fn(item, (err, result) => {
			if (err) error = err;
			results[i] = (result  as Result); // https://github.com/facebook/flow/issues/2123
 ...
 * ```
 */
export function pick(src: any, properties: string[]): any {
	const result = {};
 ...
 */
export function calculateSignedArea(ring: Point[]): number {
	let sum = 0;
...
export function readImageDataUsingOffscreenCanvas(
	imgBitmap: HTMLCanvasElement | HTMLImageElement | ImageBitmap | OffscreenCanvas,
	x: number, y: number, width: number, height: number,
): Uint8ClampedArray {
